(() => {
    ...
    function initSliders() {
        if (document.querySelector(".technique-propose__slider")) new swiper_core_Swiper(".technique-propose__slider", {
            ...
            pagination: {
                el: ".technique-propose__pagination",
                clickable: true
            },
            breakpoints: {
                0: {
        ...
        if (document.querySelector(".service-propose__slider")) new swiper_core_Swiper(".service-propose__slider", {
            modules: [ Pagination, Autoplay ],
            observer: true,
            observeParents: true,
            slidesPerView: 3,
            ...
            pagination: {
                el: ".service-propose__pagination",
                clickable: true
            },
            breakpoints: {
                0: {
...
***************************************************************************** */
    ...
    function counter(array) {
        ...
        array.forEach((element => {
            if (i < 10) element.innerHTML = `0${i}`;
            if (i >= 10) element.innerHTML = i;
            i++;
        }));
    }
    const stepsNumbers = document.querySelectorAll(".steps .item-steps__counter span");
    const stepsNumbers2 = document.querySelectorAll(".steps-product__item span");
    counter(stepsNumbers);
    counter(stepsNumbers2);
    if (document.body.clientWidth < 1110) {
        const header = document.querySelector("header");
        const phone = document.querySelector(".phone");
        const observer = new MutationObserver((mutationsList => {
            mutationsList.forEach((mutation => {
                if (mutation.type === "attributes" && mutation.attributeName === "class") phone.classList.toggle("_active");
            }));
        }));
    ...
    function documentActions(event) {
        ...
        if (document.body.clientWidth > 767.98) {
            if (targetElement.closest(".menu__link--catalog")) {
                const menuTitle = targetElement.closest(".menu__link--catalog");
                if (!menuTitle.classList.contains("_active")) {
                    const allCatalogs = document.querySelectorAll(`[data-catalog]`);
                    allCatalogs.forEach((element => {
                        element.classList.remove("_active");
                    }));
                    const allCatalogTitles = document.querySelectorAll(`[data-title-catalog]`);
                    allCatalogTitles.forEach((element => {
                        element.classList.remove("_active");
                    }));
                    document.documentElement.classList.remove("lock");
                }
                document.querySelectorAll(`[data-catalog]`);
                if (menuTitle.hasAttribute("data-title-catalog")) {
                    const currenCatalogId = menuTitle.dataset.titleCatalog;
                    if (currenCatalogId) {
                        const currentCatalog = document.querySelector(`[data-catalog="${currenCatalogId}"]`);
                        currentCatalog.classList.toggle("_active");
                        targetElement.classList.toggle("_active");
                        document.documentElement.classList.toggle("lock");
                    }
                }
            }
            if (!targetElement.closest(".catalog-header__catalog") && !targetElement.closest(".menu__link--catalog")) {
                const allCatalogs = document.querySelectorAll(`[data-catalog]`);
                allCatalogs.forEach((element => {
                    element.classList.remove("_active");
                }));
        ...
        if (document.body.clientWidth < 767.98) {
            ...
            if (targetElement.closest(".catalog-header__title")) {
                const currentSubMenu = targetElement.closest(".catalog-header__item");
                const neededSubMenu = currentSubMenu.querySelector(".catalog-header__title + .catalog-header__sub-block");
                console.log(neededSubMenu);
                neededSubMenu.classList.add("_active");
            }
            if (targetElement.closest(".adapt-catalog-header__undo")) {
                const currentSubMenu = targetElement.closest(".catalog-header__sub-block");
                if (currentSubMenu) currentSubMenu.classList.remove("_active"); else if (targetElement.closest(".adapt-catalog-header__undo")) {
                    const currentMenu = targetElement.closest(".catalog-header__catalog");
                    if (currentMenu) currentMenu.classList.remove("_active");
                }
            }
            if (targetElement.closest(".icon-menu")) {
                const allSubBlocks = document.querySelectorAll(".catalog-header__sub-block");
                const allCatalogs = document.querySelectorAll(".catalog-header__catalog");
                allSubBlocks.forEach((element => {
                    element.classList.remove("_active");
                }));
                allCatalogs.forEach((element => {
                    element.classList.remove("_active");
    ...
    document.querySelectorAll(".catalog-header__title a").forEach((link => {
        link.addEventListener("click", (function(event) {
            event.preventDefault();
        }));
    }));
    const pagination = document.querySelector(".pages-pagination--category");
    if (pagination) {
        const paginationMainParent = pagination.closest(".category");
        const paginationItems = paginationMainParent.querySelectorAll(".category__item");
        if (paginationItems.length < 10) pagination.style.display = "none";
    } else {
        const pagination = document.querySelector(".pages-pagination--catalog");
        if (pagination) {
            const paginationMainParent = pagination.closest(".catalog__content");
            const paginationItems = paginationMainParent.querySelectorAll(".item-technique");
            if (paginationItems.length < 10) pagination.style.display = "none";
        }
    }
    window["FLS"] = true;
